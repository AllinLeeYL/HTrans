...
class Node(object):
    ...
    def show(self, buf=sys.stdout, offset=0, attrnames=False, showlineno=True):
        ...
        if hasattr(self, 'cal_rp'):
            buf.write(' rp:' + str(self.cal_rp()))

        if hasattr(self, 'ap'):
            buf.write(' ap:' + str(self.ap))
    ...
    def calculate(self) -> None:
        ...
        p = 1.0
        while True:
            self.cal_ap(identifiers, p)
            old_identifiers = identifiers
            self.cal_ap(identifiers, p)
            for key in identifiers.keys():
                if old_identifiers[key] != identifiers[key]:
                    break
            else:
                break
            continue
...
class IntConst(Constant):
    def __init__(self, value:str, lineno=0):
        self.lineno = lineno
        self.value = value
    def cal_rp(self) -> float:
        "calculate relative possibility"
        if hasattr(self, 'rp'):
            return self.rp
        if re.match(r'[1-9][0-9]*\'[hbod][abcdef1234567890]+', self.value):
            width = int(self.value.split('\'')[0])
        elif re.match(r'([1-9][0-9]*|[0-9])', self.value):
            width = len(self.value)
        else:
            print('value:', self.value, 'cannnot be parsed!')
            raise Exception
        self.rp = 1 / width
        return self.rp
    def cal_ap(self, identifiers: dict, p: float) -> float:
        return self.cal_rp() * p
...
class Lvalue(Node):
    ...
    def cal_ap(self, identifiers: dict, p: float) -> float:
        return self.var.cal_ap(identifiers, p)
...
class UnaryOperator(Operator):
    ...
    def cal_ap(self, identifiers: dict, p: float) -> float:
        if hasattr(self, 'ap'):
            return self.ap
        self.ap = self.right.cal_ap(identifiers, p)
        return self.ap
...
class Eq(Operator):
    ...
    def cal_rp(self):
        if hasattr(self, 'rp'):
            return self.rp
        left_rp = self.left.cal_rp() if hasattr(self.left, 'cal_rp') else 1.0
        right_rp = self.right.cal_rp() if hasattr(self.right, 'cal_rp') else 1.0
        self.rp = min(left_rp, right_rp)
        return self.rp
    def cal_ap(self, identifiers: dict, p: float) -> float:
        if hasattr(self, 'ap'):
            return self.ap
        self.ap = min(self.left.cal_ap(identifiers, p), self.right.cal_ap(identifiers, p))
        return self.ap
...
class NotEq(Operator):
    ...
    def cal_rp(self):
        ...
        left_rp = self.left.cal_rp() if hasattr(self.left, 'cal_rp') else 1.0
        right_rp = self.right.cal_rp() if hasattr(self.right, 'cal_rp') else 1.0
        self.rp = 1 - min(left_rp, right_rp)
        return self.rp
    def cal_ap(self, identifiers: dict, p: float) -> float:
        if hasattr(self, 'ap'):
            return self.ap
...
class Or(Operator):
    ...
    def cal_rp(self) -> float:
        ...
        return self.rp
    def cal_ap(self, identifiers: dict, p: float) -> float:
        left_ap = self.left.cal_ap(identifiers, p)
        right_ap = self.right.cal_ap(identifiers, p)
        self.ap = left_ap + right_ap - (left_ap * right_ap)
        return self.ap
